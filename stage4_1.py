...
SUMMARY_OUTPUT_PATH = ROOT_DIR / "stage4_1_policy_summary.csv"
FTL_OUTPUT_PATH = ROOT_DIR / "stage4_1_ftl_by_type.csv"
FAIRNESS_OUTPUT_PATH = ROOT_DIR / "stage4_1_fairness_by_type.csv"
REQUEST_OUTPUT_PATH = ROOT_DIR / "stage4_1_request_results.csv.gz"
LATENCY_FIGURE_PATH = ROOT_DIR / "stage4_1_policy_latency_comparison.png"
FTL_FIGURE_PATH = ROOT_DIR / "stage4_1_ftl_by_type.png"
WAIT_FIGURE_PATH = ROOT_DIR / "stage4_1_wait_by_type.png"
FAIRNESS_FIGURE_PATH = ROOT_DIR / "stage4_1_max_wait_by_type.png"
...
# WS-MAS parameters
# Lower score is better. Requests gain priority as their wait/service ratio grows.
MODALITY_WEIGHT = 1.0
SIZE_WEIGHT = 0.15
AGING_WEIGHT = 0.35
...
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for small-batch validation runs."""
    parser = argparse.ArgumentParser(
        description="Stage 4.1 WS-MAS scheduling evaluation."
    )
    parser.add_argument(
        "--limit",
        type=int,
...
def classify_request_types(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["modality_cost"] = np.select(conditions, modality_cost, default=2.0).astype(float)
...
def simulate_modality_priority(df: pd.DataFrame) -> pd.DataFrame:
    ...
    while completed < n:
        ...
        for priority in (1, 2, 3):
            if queues[priority]:
                selected_idx = queues[priority].pop(0)
                break
...
# ---------------------- 4. WS-MAS ----------------------
def simulate_ws_mas(df: pd.DataFrame) -> pd.DataFrame:
    """
    Waiting-Sensitive Modality-Aware Scheduling.

    Score_i(t) =
        MODALITY_WEIGHT * modality_cost_i
        + SIZE_WEIGHT * normalized_service_time_i
        - AGING_WEIGHT * wait_i(t) / service_time_i

    The policy keeps text/light jobs ahead by default, but allows long-waiting
    heavy jobs to gradually gain priority instead of using abrupt promotions.
    """
    arrivals = df["arrival_time"].to_numpy(dtype=float)
    service = df["service_time"].to_numpy(dtype=float)
    modality_cost = df["modality_cost"].to_numpy(dtype=float)
    n = len(df)
    ...
    normalized_service = service / (service.mean() + EPSILON)
    ...
    def append_arrivals_up_to(now: float) -> None:
        nonlocal next_arrival_idx
        while next_arrival_idx < n and arrivals[next_arrival_idx] <= now:
            waiting.append(next_arrival_idx)
            next_arrival_idx += 1

    def compute_score(req_idx: int, now: float) -> float:
        wait_ratio = max(0.0, now - arrivals[req_idx]) / (service[req_idx] + EPSILON)
        return (
            MODALITY_WEIGHT * modality_cost[req_idx]
            + SIZE_WEIGHT * normalized_service[req_idx]
            - AGING_WEIGHT * wait_ratio
        )

    def reorder_waiting(now: float) -> None:
        waiting.sort(key=lambda req_idx: (compute_score(req_idx, now), req_idx))

    while completed < n:
        if not waiting and next_arrival_idx < n:
            current_time = max(current_time, arrivals[next_arrival_idx])
            append_arrivals_up_to(current_time)
            reorder_waiting(current_time)
        ...
        append_arrivals_up_to(current_time)
        if next_arrival_idx == n or waiting:
            reorder_waiting(current_time)
            since_rebuild = 0

    return build_policy_frame(df, "WS-MAS", starts, completion, wait)
...
def summarize_ftl_by_type(results: pd.DataFrame) -> pd.DataFrame:
    ...
    policy_order = ["FCFS", "Static Modality Priority", "WS-MAS"]
...
def plot_policy_summary(summary_table: pd.DataFrame) -> None:
    ...
    ax.set_title("Stage 4.1 Policy Comparison", pad=16)
    ax.set_xlabel("")
...
def main() -> None:
    ...
    print("=== Stage 4.1: WS-MAS Scheduling Evaluation ===")
    ...
    print("\nWS-MAS parameters:")
    print(f"  Modality weight: {MODALITY_WEIGHT:.2f}")
    print(f"  Size weight:     {SIZE_WEIGHT:.2f}")
    print(f"  Aging weight:    {AGING_WEIGHT:.2f}")
    print(f"  Rebuild every:   {REBUILD_INTERVAL} dispatches")

    fcfs_results = simulate_fcfs(trace_df)
    static_priority_results = simulate_modality_priority(trace_df)
    ws_mas_results = simulate_ws_mas(trace_df)
    all_results = pd.concat(
        [fcfs_results, static_priority_results, ws_mas_results],
        ignore_index=True,
    )
